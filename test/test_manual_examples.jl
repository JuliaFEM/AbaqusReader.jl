...
@testset "ABAQUS Manual Examples - Model Parsing" begin
    ...
    @testset "Multi-material advanced features" begin
        ...
        # Element-set-based SURFACE definitions now fully supported
        ...
        # Validate surfaces - element-set-based definitions should be expanded
        @test haskey(model.mesh.surface_sets, "TOP_SURFACE")
        @test length(model.mesh.surface_sets["TOP_SURFACE"]) == 2  # ZONE_A + ZONE_B
        @test (1, :S3) in model.mesh.surface_sets["TOP_SURFACE"]  # Element 1 from ZONE_A
        @test (2, :S3) in model.mesh.surface_sets["TOP_SURFACE"]  # Element 2 from ZONE_B
